...
def resolve_device(preference: str = "auto") -> str:
    ...
    `THALWEG_DEVICE` overrides everything, which is what makes a constrained CI
    ...
    """
    override = os.environ.get("THALWEG_DEVICE")
    if override:
        preference = override
    ...
    warnings.warn(
        ...
        f"{devices}. Falling back to {devices[0]!r}. Set THALWEG_DEVICE to pin "
        "a device explicitly.",
...
def check_memory_budget(n_windows: int, lookback: int, n_features: int,
    ...
    `THALWEG_MEMORY_LIMIT_GB` environment variable, or 8 GB, which is a
    conservative laptop figure rather than a measurement of this machine.
    """
    if limit_gb is None:
        limit_gb = float(os.environ.get("THALWEG_MEMORY_LIMIT_GB", "8"))
    ...
    raise MemoryError(
        ...
        f"  Raise the limit:  THALWEG_MEMORY_LIMIT_GB={needed_gb:.0f}\n"
