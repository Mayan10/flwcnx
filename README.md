...
(LEO satellite) access links, from the research package to a running console.**

[![CI](https://github.com/Mayan10/thalweg/actions/workflows/ci.yml/badge.svg)](https://github.com/Mayan10/thalweg/actions/workflows/ci.yml)
...
> **thalweg** *(n.)* the line of lowest points along a channel: the floor of a
> riverbed, and the reference a survey is taken against.
...
- [The platform around it](#the-platform-around-it)
...
```bash
git clone https://github.com/Mayan10/thalweg.git
cd thalweg
...
pytest -q                               # 323 tests, no dataset and no GPU needed
```
...
python -m thalweg.api.server --mode synthetic --port 8010  # stream decisions live
```

## Repository layout

The research package is the repository. Everything else is built around it.

| Path | What it is | Stack |
|---|---|---|
| [`thalweg/`](thalweg) | **The package.** Ingest, features, forecast, calibration, decisions. | Python 3.11+, PyTorch |
| [`thalweg/api/`](thalweg/api) | Streams live decisions out of the real pipeline over WebSocket | FastAPI |
| [`scripts/`](scripts) | One entry point per experiment, plus the figure and table generators | Python |
| [`tests/`](tests) | 323 tests on synthetic fixtures, no dataset and no GPU | pytest |
| [`docs/`](docs) | Paper, report, limitations, progress log, bibliography, figures | Markdown |
| [`results/summary/`](results/summary) | Every committed table, each traceable to a run and its config snapshot | Markdown |
| [`services/backend/`](services/backend) | Accounts, API keys and company records | Node, Express, Prisma, Postgres |
| [`services/webui/`](services/webui) | Web console: landing page and the live decision dashboard | Next.js, TypeScript |
| [`services/tui/`](services/tui) | Terminal console over the same decision stream | Rust, ratatui |
| [`Dockerfile`](Dockerfile), [`docker-compose.yml`](docker-compose.yml) | The ML service image, and the whole stack | Docker |
...
| **Protection layer** | [`thalweg/decide/flows.py`](thalweg/decide/flows.py) and [`thalweg/decide/protect.py`](thalweg/decide/protect.py) - the scorer and the allocator, both documented in full at the top of the file |
...
contending for them are generated by `thalweg/eval/workload.py`. Every number in
...
thalweg/
  ...
  api/                 the FastAPI server the consoles read
docs/
  figures/             the sixteen README figures, 300 DPI, regenerated from runs
  paper/               the paper and the report
  limitations.md       written while building, not retrofitted
  progress.md          one entry per phase, including the withdrawn claims
scripts/               one entry point per experiment, plus figure and table generators
tests/                 323 tests, synthetic fixtures only
results/summary/       the committed tables (the rest of results/ is gitignored)
data/                  gitignored
```

Every layer only talks to the one below it, and `eval/` sits beside all of them.
That separation is what makes the pipeline testable: the whole suite runs on
synthetic fixtures with no dataset and no GPU.
...
**Device.** `thalweg/device.py` picks CUDA, then MPS, then CPU. An explicit
...
```bash
THALWEG_DEVICE=cpu python scripts/run_demo.py --location canada   # pin a device
```
...
```bash
pytest -q                                               # 323 tests, synthetic fixtures
...
python -m thalweg.eval.runner --location usa --stride 6  # the full calibration grid
...
from thalweg.decide.protect import ProtectionController
...
A web console, a terminal console and a small accounts service, all reading one
decision stream out of the real pipeline. None of it is a mock: `--mode` only
chooses which trace is replayed.

```
                        thalweg  (the research package)
...
```bash
python -m pip install -e ".[api]"                         # the ML API
python -m thalweg.api.server --mode synthetic --port 8010

cd services/tui && cargo run --release                    # terminal console
cd services/backend && npm install && npm run dev         # port 3001
cd services/webui   && npm install && npm run dev         # port 3000
```

In the terminal console: `h` home, `k` login, `m` the live ML dashboard, `/`
command bar, `q` quit. The web console's defaults already point at
`localhost:3001/api` and `localhost:8010`, so no environment file is needed for
local development; override with `NEXT_PUBLIC_API_URL` and
`NEXT_PUBLIC_ML_WS_URL`, which Next inlines at build time.

[`services/tui/connection.md`](services/tui/connection.md) documents the stream
protocol in full, and [`docs/demo-runbook.md`](docs/demo-runbook.md) is the
runbook for presenting it: what to start, in what order, what to point at, and
what to say about the numbers before someone asks.

### What the platform is not

It is a working demonstration rather than a deployment, and the gaps are worth
...
- `depends_on` in compose orders startup but does not wait for readiness, so
  ...
  trains.
- Both consoles read the stream; neither can change the running configuration.
